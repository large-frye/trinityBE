...
package controllers.javascript {
  ...
  class ReverseWorkorders(_prefix: => String) {
    ...
    def findByDate: JavaScriptReverseRoute = JavaScriptReverseRoute(
      ...
      """
        function(filterType,inspectionType,limit,start,amount) {
          return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "workorders/date/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("filterType", encodeURIComponent(filterType)) + "/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("inspectionType", encodeURIComponent(inspectionType)) + "/" + (""" + implicitly[PathBindable[Int]].javascriptUnbind + """)("limit", limit) + "/" + (""" + implicitly[PathBindable[Int]].javascriptUnbind + """)("start", start) + "/" + (""" + implicitly[PathBindable[Int]].javascriptUnbind + """)("amount", amount)})
        }
      """
    )
