...
class Routes extends GeneratedRouter {
  ...
  def documentation: Seq[(String, String, String)] = List(
    ...
    ("""GET""", prefix + (if(prefix.endsWith("/")) "" else "/") + """workorders/date/$filterType<[^/]+>/$inspectionType<[^/]+>/$limit<[^/]+>/$start<[^/]+>/$amount<[^/]+>""", """controllers.Workorders.findByDate(filterType:String, inspectionType:String, limit:Int, start:Int, amount:Int)"""),
  ...
  private[this] lazy val controllers_Workorders_findByDate4_route: Route.ParamsExtractor = Route("GET",
    PathPattern(List(StaticPart(this.prefix), StaticPart(this.defaultPrefix), StaticPart("workorders/date/"), DynamicPart("filterType", """[^/]+""",true), StaticPart("/"), DynamicPart("inspectionType", """[^/]+""",true), StaticPart("/"), DynamicPart("limit", """[^/]+""",true), StaticPart("/"), DynamicPart("start", """[^/]+""",true), StaticPart("/"), DynamicPart("amount", """[^/]+""",true)))
  )
  private[this] lazy val controllers_Workorders_findByDate4_invoker = createInvoker(
    controllers.Workorders.findByDate(fakeValue[String], fakeValue[String], fakeValue[Int], fakeValue[Int], fakeValue[Int]),
    HandlerDef(this.getClass.getClassLoader,
      "router",
      "controllers.Workorders",
      "findByDate",
      Seq(classOf[String], classOf[String], classOf[Int], classOf[Int], classOf[Int]),
      "GET",
      """""",
      this.prefix + """workorders/date/$filterType<[^/]+>/$inspectionType<[^/]+>/$limit<[^/]+>/$start<[^/]+>/$amount<[^/]+>"""
    )
  )
  ...
  def routes: PartialFunction[RequestHeader, Handler] = {
    ...
    case controllers_Workorders_findByDate4_route(params) =>
      call(params.fromPath[String]("filterType", None), params.fromPath[String]("inspectionType", None), params.fromPath[Int]("limit", None), params.fromPath[Int]("start", None), params.fromPath[Int]("amount", None)) { (filterType, inspectionType, limit, start, amount) =>
        controllers_Workorders_findByDate4_invoker.call(controllers.Workorders.findByDate(filterType, inspectionType, limit, start, amount))
      }
